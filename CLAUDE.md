...
- **Interactive price chart layout** (`chart_generator.py`): 4-panel Plotly chart; `vertical_spacing=0.05`, `height=700`, `margin t=110`. Legend is centred (`x=0.5, xanchor="center"`) at `y=1.12`. Action annotation (BUY/SELL/HOLD/NO_TRADE) is in paper coords at `x=0.5, y=1.07`, centred above the panel title. Outer chart wrapped in a 3px `#5b7fa6` bordered div. Separator lines drawn at `domains[i][0]` (bottom edge of each upper panel) to avoid crossing subplot titles. S/R levels are re-classified at render time against the current close price (below close → green Support, above close → red Resistance).
